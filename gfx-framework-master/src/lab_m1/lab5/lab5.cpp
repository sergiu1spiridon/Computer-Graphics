...
void Lab5::Update(float deltaTimeSeconds)
{
...

    {
        ...
        modelMatrix = glm::rotate(modelMatrix, RADIANS(215.f), glm::vec3(0.5f, 1.f, 0));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(1.f, 0.4f, 0.7f));
        RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
    }
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(4.27f, 2.5f, 2.5f));
        modelMatrix = glm::rotate(modelMatrix, RADIANS(245.f), glm::vec3(0.315f, 0.4f, 0.2f));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(.6f, 1.f, .9f));
        RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
    }
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    {
        fieldOfView += deltaTime * cameraSpeed;
        if (fovType == true)
        {
...
void Lab5::OnKeyPress(int key, int mods)
{
    ...
    {
        fovType = false;
        projectionMatrix = glm::ortho(.02f, 10.f, .05f, 10.f, 0.01f, 200.f);
    }
    if (key == GLFW_KEY_P)
    {
        fovType = true;
        projectionMatrix = glm::perspective(RADIANS(fovType), window->props.aspectRatio, 0.01f, 200.f);
    }
